...
class ContactController extends JController
{
	...
	function submit()
	{
		...
		if (!$pparams->get( 'custom_reply' ))
		{
			...
			$body 	= $prefix."\n".$name.' <'.$email.'>'.$phone."\r\n\r\nJeg ønsker at vide mere om:".stripslashes($str)."\r\n\r\n".stripslashes($body);

			$mail = JFactory::getMailer();

			$mail->addRecipient( $contact->email_to );
			$mail->setSender( array( $email, $name ) );
			$mail->setSubject( $FromName );
			$mail->setBody( $body );
		...
		$link = JRoute::_('index.php?option=com_contact&view=contact&id='.$contact->slug.'&catid='.$contact->catslug.'&success=1', false);
