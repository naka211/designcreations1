...
function mosGetMenuLink($mitem, $level = 0, & $params, $open = null)
{
	...
	switch ($mitem->browserNav)
	{
		...
		default : // formerly case 2
			...
			$txt = '<a href="' . $mitem->url . '" ' . $id . '><span>' . $mitem->name . '</span></a>';
			break;
	}
		...
		switch ($style)
		{
			case 1 :
				...
				foreach ($links as $link) {
					...
					if($index == 1) { 
						echo '<li class="first '.$current.'">' . $link . '</li>';
					} else if($index == count($links)) {
						echo '<li class="last '.$current.'">' . $link . '</li>';
					} else {
						echo '<li '.$current1.'>' . $link . '</li>';
					}
